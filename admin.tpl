...
jQuery(document).ready(function(){
  ...
  jQuery("#addinfoClose").click(function(){
   ...
   jQuery("#addinfoClose2").click(function(){
    window.location.href = "admin.php?page=user_list";
    return false;
  });
...
{if isset ($gestImp)}
	<form method="post" >
		<fieldset>
			<legend>{'Import infos since Add Info Users'|@translate}</legend>
			<input class="submit" name="submitimport" type="submit" value="{'Import'|@translate}">
		</fieldset>
	</form>
{/if}
...
{if isset ($addinfotemplate)}
        <p class="showCreateAlbum">
            <a href="#" id="aip_sumit" >{'Create new custom fields'|@translate} </a>
        </p>
    <div id="add_info_edit" style="display: none;">
        <form method="post" >
            <fieldset>
                <legend><span id="leg_add">{'Create new custom fields'|@translate}</span><span id="leg_edit">{'Edit custom fields'|@translate}</span></legend>
                <input id="hideid" type="hidden" name="invisibleID" value="">
                <p class="input">
				...
				<p class="input" style="width: 700px;">
                    ...
                    <input id="aip_adminonly" type="checkbox" name="adminonly" value="1">
                </p>
				<p class="input" style="width: 700px;">
                    <label for="obligatory">{'Obligatory'|@translate}</label>
                    <input id="aip_obligatory" type="checkbox" name="obligatory" value="1">
                </p>
...
<form method="post" name="ucfprofile" id="ucfprofile" class="properties">
 ...
 <fieldset>
    ...
    <ul>
      {foreach from=$tab_user_custom_fields_adminlist item=userinfo}
		...
		{else if}
		  <li>
			<span class="property">
              <label for="data[{$userinfo.UCFID}]">{$userinfo.UCFWORDING}</label>
			</span>
  ...
  <p class="actionButtons">
	...
	<a href="#" id="addinfoClose2">{'Cancel'|@translate}</a>
  </p>
 </fieldset>
</form>
{/if}